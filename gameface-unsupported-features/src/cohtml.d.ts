...
interface Engine {
    ...
    * @param name The name of the event
    * @param callback The callback to be executed, when there isn't any registered JS or C++ handler for that event
    **/
    mockEvent(name: string, callback?: ArbitraryCallback): void;

    /**
    * Register a mock handler for a Call, that will be invoked if there is no C++ handler
    * @param name The name of the handler to be invoked
    * @param callback The callback to be executed, when there is no registered C++ handler
    **/
    mockCall(name: string, callback?: ArbitraryCallback): void;
    ...
    * @param includeInherited Whether to include both own and inherited enumerable properties of `jsObject`
    **/
    createOrMergeModel(name: string, jsObject: object, includeInherited: boolean = true): void;
